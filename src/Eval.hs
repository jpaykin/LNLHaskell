...
eval :: Domain dom => LExp dom 'Empty s -> DomEffect dom (LExp dom 'Empty s)
eval e = fmap valToExp $ eval' e


eval' :: forall dom s. Domain dom => LExp dom 'Empty s -> DomEffect dom (LVal dom s)
eval' (Dom e)           = evalDomain @dom e
