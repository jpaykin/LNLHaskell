...
class HasMILL sig => HasByteString sig where
  ...
  free :: LExp sig '[] (Ptr ⊸ One)

  -- memcopy ::  Int ⊸ Int ⊸ Int ⊸ Ptr ⊸ Ptr ⊸ Ptr ⊗ Ptr
  -- memcopy o1 o2 len p1 p2 replicates p1+o1+length bytes into p2+o2
  memcopy :: Int -> Int -> Int -> LExp sig '[] (Ptr ⊸ Ptr ⊸ Ptr ⊗ Ptr)
  memmove :: Int -> Int -> Int -> LExp sig '[] (Ptr ⊸ Ptr ⊸ Ptr)
  memset :: Int -> Int -> Word8 -> LExp sig '[] (Ptr ⊸ Ptr)

  empty   :: LExp sig '[] Buffer
  size    :: LExp sig '[] (Buffer ⊸ Buffer ⊗ Lower Int)
  is_null :: LExp sig '[] (Buffer ⊸ Buffer ⊗ Lower Bool)
  head    :: LExp sig '[] (Buffer ⊸ Buffer ⊗ Lower (Maybe Word8))
  cons    :: Word8 -> LExp sig '[] (Buffer ⊸ Buffer)
  append  :: LExp sig '[] (Buffer ⊸ Buffer ⊸ Buffer ⊗ Buffer)
  free_buffer :: LExp sig '[] (Buffer ⊸ One)
