...
data ExistsArray f α where -- = ExistsArray { new :: exp Empty (Array token α) }
...
instance HasArray Shallow where
  ...
  slice i e = SExp $ \γ -> do 
        ...
        return $ VPair ( VArray (filter (\j -> j < i) bounds) arr )
                       ( VArray (filter (\j -> j >= i) bounds) arr )
...
partition :: (HasArray sig,Ord α) 
          => [Int] -> LStateT sig (Array token α) Int
