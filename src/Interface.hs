...
var :: forall x t f1 f2 g. CSingletonCtx x t f1 f2 g 
    => LExp g t
var = Var (singletonCtx @x @t @f1 @f2)

λ :: forall x s f1 f2 t g g'. CAddCtx x s f1 f2 g g' 
  => LExp g' t -> LExp g (s ⊸ t)
...
put :: forall (f::Frame) (g::Ctx) a. CEmptyCtx f g => a -> LExp g (Lower a)
put a = Put @g @f (emptyCtx @f @g) a
...
data Lift :: Frame -> LType -> * where
  Suspend :: forall f g t. CEmptyCtx f g => LExp g t -> Lift f t
...
coerce :: forall f g1 g2 t. (CEmptyCtx f g1, CEmptyCtx f g2) => LExp g1 t -> LExp g2 t
...
force :: forall f g t. CEmptyCtx f g => Lift f t -> LExp g t
-- e :: LExp g t
...
suspend :: forall f g a. CEmptyCtx f g => LExp g a -> Lift f a
suspend = Suspend

suspendL :: forall f g a. CEmptyCtx f g => LExp g (Lower a) -> Lin a
